...
public class TestTypedSchemaRepository {
      ...
      for (Converter idConverter : idConverters) {
        for (Converter schemaConverter : schemaConverters) {
          for (Converter subjectConverter : subjectConverters) {
            testRegistration(repoProvider.createRepository(), idConverter, schemaConverter, subjectConverter);
  ...
  private <ID, SCHEMA, SUBJECT> void testRegistration(Repository innerRepo, Converter<ID> convertId,
    ...
    try {
      ...
      Assert.assertNull(testName + "Latest schema should be null before being registered.", latestSchemaForSubject1);
      ...
      Assert.assertEquals(testName + "getSchema for 1st ID should work.", subject1Schema1, schema1ForSubject1ById);

      SCHEMA schema2ForSubject1ById = repo.getSchema(subject1, idForSubject1Schema2);
      Assert.assertEquals(testName + "getSchema for 2nd ID should work.", subject1Schema2, schema2ForSubject1ById);
      ...
      Assert.fail(testName + "A SchemaValidationException was thrown during testRegistration: " + e.getMessage());
    } finally {
      try {
        innerRepo.close();
      } catch (IOException e) {
