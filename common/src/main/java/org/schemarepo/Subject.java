 ...
 * <li>{@link #register(String)} attempts to register a schema with the Subject,
 ...
 * <li>{@link #registerIfLatest(String, SchemaEntry)} attempts to register a
 * schema with the Subject, according to the validation rules of the Subject.
 * The operation succeeds only if the provided {@code latest} value is the
 * current latest schema in the system, and returns null otherwise.</li>
 * <li>{@link #lookupById(String)} looks up a schema by its id, and returns null
 * if such a schema does not exist. Since the mapping from id to schema is
 * immutable, this result is cacheable.</li>
 * <li>{@link #lookupBySchema(String)} looks up an id for a schema, and returns
 * null if no such schema exists. Since the mapping from id to schema is
 * immutable, this result is cacheable.</li>
 * <li>{@link #allEntries()} returns all the schema entries for the subject,
 * ordered from most recent to oldest. The result is not cacheable, since
 * additional entries may be added.</li>
 *
 */
public abstract class Subject {
  ...
  /**
   * A {@link Subject} has a name. The name must not be null or empty, and cannot
   * contain whitespace. If the name contains whitespace an
  ...
  public static Subject validatingSubject(Subject subject, ValidatorFactory factory) {
    ...
    // if the validators key is not specified in the subject config, get the default
    // ones.
   ...
   *
   * @param subject The {@link Subject} to wrap
   * @param cache   The {@link SchemaEntryCache} to cache with
   ...
   *         not null. <br/>
   *         If the provided subject is null, returns null. If the provided cache
   *         is null, returns the provided subject without wrapping it.
   */
   ...
   * both get the same {@link SchemaEntry} result and succeed.
   *
   * @param schema The schema to register
   * @return The id of the schema
   * @throws SchemaValidationException If the schema change is not valid according
   *                                   the validation rules of the subject
   */
  public abstract SchemaEntry register(String schema) throws SchemaValidationException;
   ...
   *                                   the validation rules of the subject
   */
  public abstract SchemaEntry registerIfLatest(String schema, SchemaEntry latest) throws SchemaValidationException;
  ...
  /**
   * Lookup the {@link SchemaEntry} for the given subject by id. Since the mapping
   * of schema to id is immutable the result can be cached.
   *
   ...
   *
   * @return The {@link SchemaEntry} or null if no schema is registered with this
   *         subject
   */
   ...
   * recent to oldest. This result is not cacheable, since the {@link SchemaEntry}
   * in the subject may grow over time.
   *
